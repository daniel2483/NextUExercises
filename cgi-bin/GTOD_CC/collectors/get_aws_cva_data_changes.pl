...
#
###############################################################
my $ci_metric_cmd = qq( /usr/local/bin/aws athena start-query-execution --query-string "select distinct a.ci_alias_nm,
			c.ci_sk as ci_id,
			c.data_dmn_nm as data_domain_nm,
			c.co_sk as client_id,
			c.co_nm as client_alias_nm,
			date_trunc('day',mtrc_utc_ts),
			m.ci_type_nm as ci_alias_type,
			m.ci_nm as metricset_nm,
			m.natv_mtrc_cd,
			m.natv_mtrc_valu_nr,
			m.mtrc_smpl_ct as metric_sample_ct,
			m.mtrc_mod_nm as metricset_module,
			c.src_clnt_cd,
			m.src_sys_nm
from transformed.ci_mtrc_r m
JOIN transformed.ci_d c ON c.ci_ky = m.ci_ky
JOIN transformed.ci_attr_d a ON a.ci_ky = m.ci_ky
JOIN transformed.co_d co ON co.co_ky = c.co_ky
WHERE
			m.refresh_dt >= 'INGEST_START_DATE_GOES_HERE' AND m.refresh_dt <= 'INGEST_END_DATE_GOES_HERE'
			and mtrc_utc_ts >= date_parse('START_DATE_GOES_HERE', '%Y-%m-%d %H:%i:%s') and mtrc_utc_ts <= date_parse('END_DATE_GOES_HERE', '%Y-%m-%d %H:%i:%s')
GROUP BY a.ci_alias_nm,
			c.ci_sk,
			c.data_dmn_nm,
			c.co_sk,
			c.co_nm,
			...
			m.mtrc_mod_nm,
			c.src_clnt_cd,
			m.src_sys_nm" --result-configuration "OutputLocation=s3://aws-athena-query-results-'ACCOUNT_GOES_HERE'-'REGION_GOES_HERE'/OCD2/" --region 'REGION_GOES_HERE' --profile 'ROLE_GOES_HERE');
...
#
###############################################################
my $ci_resource_metric_cmd = qq( /usr/local/bin/aws athena start-query-execution --query-string "select distinct a.ci_alias_nm,
			c.ci_sk as ci_id,
			c.data_dmn_nm as data_domain_nm,
			c.co_sk as client_id,
			c.co_nm as client_alias_nm,
			...
			m.ci_type_nm as ci_alias_type,
			m.ci_nm as metricset_nm,
			m.resrc_type_cd as resource_type,
			m.resrc_nm as resource_nm,
			m.natv_mtrc_cd,
			m.natv_mtrc_valu_nr,
			m.mtrc_smpl_ct as metric_sample_ct,
			m.mtrc_mod_nm as metricset_module,
			c.src_clnt_cd,
			m.src_sys_nm
from transformed.ci_resrc_mtrc_r m
JOIN transformed.ci_d c ON c.ci_ky = m.ci_ky
JOIN transformed.ci_attr_d a ON a.ci_ky = m.ci_ky
JOIN transformed.co_d co ON co.co_ky = c.co_ky
WHERE
...
GROUP BY a.ci_alias_nm,
			...
			c.src_clnt_cd,
			m.src_sys_nm,
			m.natv_mtrc_valu_nr" --result-configuration "OutputLocation=s3://aws-athena-query-results-'ACCOUNT_GOES_HERE'-'REGION_GOES_HERE'/OCD2/" --region 'REGION_GOES_HERE' --profile 'ROLE_GOES_HERE');
...
sub build_aws_queries {
	...
	foreach my $env (@aws_environments){
		if (not defined ($opts{"t"}) or (defined($opts{"t"}) and $opts{"t"} =~ /avcap/)){
			...
			$query =~ s/\'ACCOUNT_GOES_HERE\'/$aws_accounts{$env}/g;
			$query =~ s/\'REGION_GOES_HERE\'/$aws_regions{$env}/g;
			my $key = $env . "_AWS_RES_METRICS_TEST";
			$aws_queries{$key}{QUERY} = $query;
			$aws_queries{$key}{REGION} = $aws_regions{$env};
			$aws_queries{$key}{ROLE} = $aws_roles{$env};
			$aws_queries{$key}{ACCOUNT} = $aws_accounts{$env};
			...
			#$query =~ s/\'ACCOUNT_GOES_HERE\'/$aws_accounts{$env}/g;
			#$query =~ s/\'REGION_GOES_HERE\'/$aws_regions{$env}/g;
			#my $key = $env . "_AWS_STAT_METRICS_TEST";
			#$aws_queries{$key}{QUERY} = $query;
			#$aws_queries{$key}{REGION} = $aws_regions{$env};
			#$aws_queries{$key}{ROLE} = $aws_roles{$env};
			#$aws_queries{$key}{ACCOUNT} = $aws_accounts{$env};
...
sub process_avcap{
	...
	$avcap_files{CI_METRICS}{OUT_FILE} = "$rawdata_dir/aws.ci_metric_test";
	$avcap_files{CI_RES_METRICS}{OUT_FILE} = "$rawdata_dir/aws.ci_res_metric_test";
	$avcap_files{CI_STAT_METRICS}{OUT_FILE} = "$rawdata_dir/aws.ci_stat_metric_test";

	#  Check if month argument (-m option) is set if not will process 30DAYS
	my $extract_month = uc($opts{"m"}) || "DAILY";
	...
	foreach my $env (@aws_environments){
		...
		$avcap_files{CI_METRICS}{IN_FILE}{$env} = "$rawdata_dir/aws/" . $env . "_AWS_METRICS_TEST.csv";
		$avcap_files{CI_RES_METRICS}{IN_FILE}{$env} = "$rawdata_dir/aws/" . $env . "_AWS_RES_METRICS_TEST.csv";
		#$avcap_files{CI_STAT_METRICS}{IN_FILE}{$env} = "$rawdata_dir/aws/" . $env . "_AWS_STAT_METRICS_TEST.csv";
	}

	foreach my $avcap_type (keys %avcap_files) {
		my $prev_month;

		if ($extract_month !~ /DAILY/i) {
			# Open output file
			...
			my $avcap_file_daily = $avcap_files{$avcap_type}{OUT_FILE}."_DAILY";
			my $avcap_file_prev_month = $avcap_files{$avcap_type}{OUT_FILE}."_".$prev_month;
			open (AWS_OUTFILE_30DAYS, ">$avcap_file_daily");
			open (AWS_OUTFILE, ">$avcap_file_prev_month");
			print "Writing data to $avcap_file_daily\n";
