...
sub build_aws_queries {
	...
	foreach my $env (@aws_environments){
		if (not defined ($opts{"t"}) or (defined($opts{"t"}) and $opts{"t"} =~ /avcap/)){
			...
			# Important Notes: Commented out the Ci metrics and Ci resource metrics, this is running on get_aws_cva_data_changes.pl with the new tables instead of running on this script
...
if (not defined($opts{"p"}) or (defined($opts{"p"}) and $opts{"p"} =~ /w/)){
	...
	if  (not defined($opts{"t"}) or (defined($opts{"t"}) and $opts{"t"} =~ /avcap/)){
		print "Processing CVA Availability data\n";
		process_avcap();
	}elsif(defined($opts{"t"}) and $opts{"t"} =~ /cva_enmt/){
