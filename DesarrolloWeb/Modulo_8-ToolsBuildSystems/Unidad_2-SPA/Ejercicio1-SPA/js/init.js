(function($){
  ...
  $(function(){
    ...
    // En el archivo principal JavaScript de la aplicación, inicializa el enrutador indicando
    // el nombre del directorio base del proyecto. En este ejemplo, el directorio se llama
    // enrutamiento.
    page.base('/enrutamiento_')
    // Define las rutas de la aplicación mediante el método page. Abre un bloque de paréntesis,
    // y en el primer parámetro se indica una cadena de caracteres correspondiente a la ruta,
    // y en el segundo, el nombre de la función que se debe ejecutar. Dicha función es la que
    // se encarga de alterar la estructura de la página para realizar un cambio entre cada
    // vista.
    page('/fotos-pag2', sigFotos)
    page('/index', cerrarFoto)
    page('/foto-ampliada/:foto', abrirFoto)
    // Crea una ruta adicional con el operador asterisco (*) para todas las rutas que no coincidan
    // con las definidas. En esta situación se debe ejecutar una función que informa que la
    // página a la que se accede no se encuentra, conocida comúnmente como notfound.
    page('*', notfound)
    // Al finalizar debes inicializar el enrutador mediante el método page sin argumentos.
    page()
